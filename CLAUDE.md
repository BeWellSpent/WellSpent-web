...
npm run generate     # both codegen steps below
npm run generate:proto   # proto from BSR -> src/gen/
npm run generate:rest    # OpenAPI contract -> src/gen/rest/
```
...
- **REST** (`openapi-fetch`) — a *small* second transport for the endpoints that are global and rarely-changing, so the browser can genuinely cache them. See below.
...
**REST calls:** a handful of endpoints are plain cacheable HTTP rather than
Connect — currently the status banner, the country list and the changelog.
`WellSpent-proto/openapi/README.md` has the rule for what qualifies; the short
version is *global and rarely-changing*, and every mutation stays on Connect.

```typescript
import { publicRestClient, unwrap } from '@/lib/api/rest'   // no token
import { useRestClient } from '@/hooks/useRestClient'        // authenticated
import type { StatusBanner } from '@/lib/api/restModels'

const { banner } = unwrap(await publicRestClient.GET('/rest/v1/status/banner'))
```

- **Use `publicRestClient` for anything public**, even from an authenticated
  screen. Attaching a token to a public endpoint makes the response look
  user-specific to a cache that can't know otherwise, which throws away the only
  thing this transport buys.
- **`unwrap`** turns `openapi-fetch`'s `{ data, error }` result into a value or a
  throw, so call sites read like the Connect ones.
- **Enums are string unions**, not numeric enum objects — `'critical'`, not
  `StatusBannerSeverity.CRITICAL`. They match the values stored in the database.
- Types come from `@/lib/api/restModels`, not `@/gen/rest/schema` directly.
...
- `src/gen/` — protobuf types, from `buf.build/bewellspent/wellspent`
- `src/gen/rest/` — REST types, from `WellSpent-proto`'s `openapi/v1/` (fetched
  over HTTPS; there is no BSR channel for OpenAPI)

Both are regenerated by `npm run generate`.
